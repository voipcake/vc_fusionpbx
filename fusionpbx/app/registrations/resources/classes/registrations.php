...
/*		public function get($profile) {
		...
		public function get($profile) {
				$id = 0;
				$sql = "select * from v_registrations ";
				$database = new database;
				$registrations = $database->select($sql, $parameters, 'all');

				if (is_array($registrations) && @sizeof($registrations) != 0) {
					foreach ($registrations as $row) {
						$registrations[$id]['user'] = $row['user_name'];
						$registrations[$id]['call-id'] = $row['call_id'];
						$registrations[$id]['contact'] = $row['contact'];
						$registrations[$id]['sip-auth-user'] = $row['sip_auth_user'];
						$registrations[$id]['agent'] = $row['agent'];
						$registrations[$id]['host'] = $row['host'];
						$registrations[$id]['network-port'] = $row['network_port'];
						$registrations[$id]['sip-auth-realm'] = $row['sip_auth_realm'];
						$registrations[$id]['mwi-account'] = $row['mwi_account'];
						$registrations[$id]['status'] = $row['status'];
						$registrations[$id]['ping-time'] = $row['ping_time'];
						$registrations[$id]['sip_profile_name'] = $row['sip_profile_name'];
						$registrations[$id]['network-ip'] = $row['network_ip'];
						$registrations[$id]['lan-ip'] = $row['lan_ip'];
						$id++;
					}
				}
				return $registrations;
